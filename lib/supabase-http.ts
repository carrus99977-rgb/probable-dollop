...
const fromUser = (data: any, access: string, cookies: string[], activeUntil: number): CloudSession => {
  ...
  const allowed = (process.env.DOLGI_ALLOWED_EMAILS || '').split(',').map(value => value.trim().toLowerCase()).filter(Boolean);
  if (!allowed.length) throw new CloudError('Доступ ещё не настроен владельцем сайта.', 503);
  if (!allowed.includes(data.email.toLowerCase())) throw new CloudError('Этот аккаунт не имеет доступа к приложению.', 403);
...
export async function emailSignIn(email: string, password: string, signup: boolean) {
  ...
  const checked = await supabaseRequest('/auth/v1/user', data.access_token);
  if (!checked.r.ok) throw new CloudError('Не удалось проверить аккаунт', 401);
