...
let calls: string[] = [];
try {
  globalThis.fetch = async (url, options) => { calls.push(String(url)); assert.equal(new Headers(options?.headers).get('apikey')?.startsWith('sb_publishable_'), true); return response({ access_token: 'verified-access', refresh_token: 'refresh', expires_in: 3600 }); };
