...
let source = fs.readFileSync('app/api/data/route.ts', 'utf8')
  ...
  .replace("import { siteIdentity } from '@/lib/access';", 'const siteIdentity=arguments[1];')
...
try {
  ...
  currentUser = 'alice'; siteSignedIn = false; assert.equal((await api.GET(get())).status, 401); siteSignedIn = true;
