...
let refreshes=0, revoked=false, initialLogin=true;
try {
  globalThis.fetch=async(url,options)=>{
    const path=String(url);
    if(path.includes('grant_type=password'))return Response.json({access_token:'old-access',refresh_token:'old-refresh'});
    if(path.endsWith('/auth/v1/user'))return new Headers(options?.headers).get('Authorization')=== (initialLogin ? 'Bearer old-access' : 'Bearer new-access')?Response.json(user):Response.json({}, {status:401});
  ...
  const login=await http.emailSignIn(user.email,'test-password',false); initialLogin=false;
