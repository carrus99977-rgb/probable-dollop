...
const source = fs.readFileSync('app/api/auth/github/route.ts', 'utf8')
  .replace("import { siteIdentity } from '@/lib/access';", 'const siteIdentity=arguments[0];')
...
try {
  ...
  allowed = false; response = await api.POST(req({ type: 'prepare', verifier: JSON.stringify(verifier) })); assert.equal(response.status, 401); allowed = true;
  ...
  console.log('PASS: official Supabase SDK start with mocked Auth responses -> PKCE callback -> verified session -> same-user data; cookie security, missing verifier, provider failure, origin/site guards, URL cleanup, reauth reset, cancellation and network failure');
