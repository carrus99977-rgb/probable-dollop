...
try {
  ...
  signedIn = false; r = await api.POST(request({ type: 'resend', email: credentials.email })); assert.equal(r.status, 401);
  assert.ok(logs.length > 0); assert.ok(logs.every(line => !line.includes('test@example') && !line.includes(credentials.password)));
  console.log('PASS: signup normalization, validation, actionable provider failures, safe logs, resend payload, no unauthenticated sessions, private-site gate');
